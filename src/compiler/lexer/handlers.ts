...
export function handleString(state: LexerState): void {
  ...
  if (j >= state.src.length) {
    throw { phase: "LEX" as const, message: `String opened at line ${startLine}, column ${startCol} was never closed. Add a closing double-quote (" ) before end of file.`, line: startLine, col: startCol };
  }

  state.push("STRING", s);
  state.advance(j - state.i + 1);
}
...
export function handleNumber(state: LexerState): void {
  ...
  state.push("NUMBER", parsedVal);
  state.advance(j - state.i);
}
...
export function handleWord(state: LexerState): void {
  ...
  if (KEYWORDS.has(word)) {
    state.push("KEYWORD", word);
  } else if (word in NAMED_COLORS) {
    state.push("NAMED_COLOR", word);
  } else if (SCENE_FIT_VALUES.has(word)) {
    state.push("SCENE_FIT", word as SceneFit);
  } else {
    state.push("IDENT", word);
  }
