...
export async function renderScene(
  ...
  // Logical Constraints Extraction
  ...
  const scaleModeProp = ast.props["scaleMode"];
  const scaleMode = scaleModeProp?.kind === "string" ? scaleModeProp.value : "contain";

  const app = new Application();
  await app.init({
    resizeTo:    hostElement, // Responsive target wrapper
    backgroundColor,
    autoStart:   false,
    antialias:   true,
  ...
  // Logical scene container to bridge Display coordinate vs Logical coordinates
  ...
  // Ticker handles resize validation reliably without external DOM event hooks mapping
  let lastW = -1;
  let lastH = -1;
